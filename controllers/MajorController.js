...
exports.loginUser = async function (req, res, next) {
  ...
  if (existingUser == null) {
    return res
      .status(200)
      .send({ status: 403, message: "User with this username does not exist" });
  }
  ...
  if (existingUser.is_locked) {
    return res
      ...
      .send({
        status: 403,
        message:
          "User Account has been locked please try again after some time.",
      });
  }
  existingUserCredential = await UserCredentialService.getByUser_id(
    existingUser._id
  );
  if (existingUserCredential == null) {
    return res
      ...
      .send({
        status: 403,
        message: "Password seems to have expired please reset your password.",
      });
  }
  ...
  if (is_locked) {
    return res
      ...
      .send({
        status: 403,
        message:
          "Incorrect password, too many unsuccessful_attempts please try again after 30 minutes. ",
      });
  }
  ...
  if (sessionType == null) {
    return res
      .status(200)
      .send({ status: 403, message: "There seems to be an error at our end." });
  }

  var existingUserSession = await UserSessionServices.getByUserIdSessionTypeId(
    existingUser._id,
    sessionType._id
  );
...
exports.getSalt = async function (req, res, next) {
  ...
  return res
    .status(200)
    .send({ status: "success", salt: existingUserCredential.credential.salt });
};
...
exports.uploadFile = async function (req, res, next) {
  ...
  uploadedFile.mv(fileName, async function (err) {
    if (err) {
      console.log(err);
      return res.status(200).send({ status: 400, message: "Technical error" });
    }
    ...
    try {
        console.log(__dirname.replace(/\\/g, '/') + fileName);
        const response = await axios.get('http://127.0.0.1:5000/prediction?image_path=' + encodeURIComponent(__dirname.replace(/\\/g, '/') +'/../'+ fileName));
        console.log(response.data);
        const combined_prediction = parseFloat(response.data);
        predicted_result = combined_prediction >= 50 ? "Positive" : "Negative";
        // Use the predicted_result variable as needed
      } catch (error) {
...
exports.addUser = async function (req, res, next) {
  ...
  var loggedinUserType = await RefUserTypesService.getById(
    loggedinUser.user_type_id
  );
  ...
  if (loggedinUserType.code !== "ADMIN") {
    return res
      .status(200)
      .send({ status: 403, message: "You do not have required authority." });
  }
  ...
  if (existingUser !== null) {
    return res
      .status(200)
      .send({ status: 403, message: "Mobile number is already in use." });
  }
  ...
  if (existingUser !== null) {
    return res
      .status(200)
      .send({ status: 403, message: "Email address is already in use." });
  }
  existingUser = await UsersService.getByUsername(req.body.username);
  if (existingUser !== null) {
    return res
      .status(200)
      .send({ status: 403, message: "Username is already in use." });
  }
  ...
  return res
    .status(200)
    .send({ status: "success", user: newUser, message: "Signup successful!" });
};
...
exports.deletePatientDetails = async function (req, res, next) {
  ...
  return res
    ...
    .send({
      status: "success",
      message: "Deleted patient details successfully",
    });
};
exports.deleteUser = async function (req, res, next) {
  ...
  return res
    .status(200)
    .send({ status: "success", message: "Deleted User successfully" });
};
